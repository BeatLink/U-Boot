...
int PDC_wc_to_utf8( char *dest, const int32_t code_in)
{
   int n_bytes_out;
   int32_t code = code_in;

#ifdef __U_BOOT__
   /* acs_map holds code page 437 glyph numbers, but the video console decodes
      UTF-8 and maps it back to code page 437 itself, so the code point is what
      has to go out. utf_to_cp() is the inverse of this index arithmetic. */
   if (code >= 0x80 && code <= 0xff)
       code = codepage_437[code - 0x60];
#endif
