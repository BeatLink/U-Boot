...
static int do_show(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	...
	/* Centre the window itself, borders and padding included. */
	int win_top  = (LINES - (menu_height + 4)) / 2;
	int win_left = (COLS  - (menu_width  + 4)) / 2;
	...
	mvwin(current_menu->window, win_top, win_left);
